...
bot.on("message", function (e) {
    if (e.raw_message === "来点二次元") {
        getDailyPicture(e, 'daily')
    }

    if (e.raw_message === "来点色图") {
        getDailyPicture(e, 'daily_r18')
    }
...
function getDailyPictureByNumber(e, r18, page, number) {
    var url = `https://www.pixiv.net/ranking.php?mode=${r18}&content=illust&p=${page}&format=json`
    ...
    fetch(url, {
        ...
        .then(function (data) {
            if (number != 0) {
                fetch(`https://www.pixiv.net/artworks/${data["contents"][number - 1]["illust_id"]}`)
                    .then(text => text.text())
                    .then(function (text) {
                        ...
                        if (page == 1) {
                            ...
                            for (var i = 1; i <= page; i++) {
                                reply += `https://pixiv.re/${data["contents"][number - 1]["illust_id"]}-${i}` + picture.substr(-4, 4)
                                reply += `\n`
                            }
                            e.reply(
                                [
                ...
                fetch(`https://www.pixiv.net/artworks/${data["contents"][49]["illust_id"]}`)
                    .then(text => text.text())
                    .then(function (text) {
                        var picture = text.match(/"original":"(.+?)"},"tags"/)[1]
                        ...
                        if (page == 1) {
                            ...
                            reply = `${head}\n作者：${data["contents"][49]["user_name"]}\ntitle：${data["contents"][49]["title"]}\npid：${data["contents"][49]["user_id"]}\np站链接：https://www.pixiv.net/artworks/${data["contents"][49]["illust_id"]}\n国内直连链接：`
                            for (var i = 1; i <= page; i++) {
                                reply += `https://pixiv.re/${data["contents"][49]["illust_id"]}-${i}` + picture.substr(-4, 4)
                                reply += `\n`
                            }
                            e.reply(
                                [
        ...
        fetch(url, {
            ...
            .then(function (data) {
                for (var n = 0; n < 50; n++) {
                    ...
                    dailyNumberCollection[n + (i - 1) * 50] = `https://www.pixiv.net/artworks/${data["contents"][n]["illust_id"]}`
        ...
        fetch(url, {
            ...
            .then(function (data) {
                for (var n = 0; n < 50; n++) {
                    ...
                    dailyNumberR18Collection[n + (i - 1) * 50] = `https://www.pixiv.net/artworks/${data["contents"][n]["illust_id"]}`
                    var tags = data["contents"][n]["tags"]
                    for (var j = 0; j < tags.length; j++) {
